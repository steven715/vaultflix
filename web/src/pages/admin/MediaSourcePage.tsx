  ...
  return (
    <div className="p-7">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-cream font-display tracking-tight">媒體來源管理</h1>
        <button
      ...
      {loading ? (
        ...
        <div className="text-faint text-center py-20">
          尚未設定媒體來源，請點擊右上角新增
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <NavIcon name="folder" className="w-4 h-4 text-muted shrink-0" />
                    <h3 className="text-cream font-medium truncate">{source.label}</h3>
                  ...
                  <p className="text-sm text-faint font-mono truncate">{source.mount_path}</p>
                  <p className="text-sm text-muted mt-1">{source.video_count} 部影片</p>
                </div>
                <div className="flex items-center gap-2 shrink-0 ml-4">
                  <button
                    onClick={() => setEditingSource(source)}
                    className="text-xs text-muted hover:text-cream px-2 py-1 rounded-btn transition-colors"
                  >
                    編輯
                  </button>
                  <button
                    ...
                    className="text-xs text-accent hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed px-2 py-1 rounded-btn transition-colors"
                  >
                    掃描匯入
                  </button>
                  <button
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(8,6,5,0.72)] backdrop-blur-[3px] flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-surface rounded-lg p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-semibold text-cream mb-4">新增媒體來源</h2>
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            placeholder="例如：D槽影片"
            className="w-full bg-surface-2 text-cream text-sm rounded-btn px-3 py-2 outline-none focus:ring-2 focus:ring-accent mb-3 border border-border"
            disabled={submitting}
            required
          />
          <label className="block text-sm text-muted mb-1">路徑</label>
          <input
            value={mountPath}
            onChange={(e) => setMountPath(e.target.value)}
            placeholder="/mnt/host/"
            className="w-full bg-surface-2 text-cream text-sm rounded-btn px-3 py-2 outline-none focus:ring-2 focus:ring-accent mb-1 font-mono border border-border"
            disabled={submitting}
            required
          />
          <p className="text-xs text-faint mb-3">路徑必須在 /mnt/host/ 下，對應 Docker 掛載的磁碟目錄</p>

          {error && (
            <div className="bg-fav/10 border border-fav/30 rounded-btn p-2 mb-3">
              <p className="text-sm text-fav">{error}</p>
            </div>
          )}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className="bg-accent text-accent-ink hover:brightness-110 disabled:opacity-50 text-sm px-4 py-1.5 rounded-btn"
            >
              {submitting ? '建立中...' : '建立'}
            </button>
          </div>
...
function EditSourceModal({ source, onClose, onSaved }: { source: MediaSource; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(8,6,5,0.72)] backdrop-blur-[3px] flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-surface rounded-lg p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-semibold text-cream mb-4">編輯媒體來源</h2>
        <form onSubmit={handleSubmit}>
          <label className="block text-sm text-muted mb-1">名稱</label>
          <input
            value={label}
            ...
            required
          />
          <label className="block text-sm text-muted mb-1">路徑（不可修改）</label>
          <input
            value={source.mount_path}
            disabled
            className="w-full bg-surface-2/50 text-faint text-sm rounded-btn px-3 py-2 mb-1 font-mono cursor-not-allowed border border-border/50"
          />
          <p className="text-xs text-faint mb-3">路徑不可修改。如需變更路徑，請刪除此來源後重新建立。</p>
...
function ConfirmDialog({ action, onConfirm, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(8,6,5,0.72)] backdrop-blur-[3px] flex items-center justify-center z-50" onClick={onCancel}>
      <div className="bg-surface rounded-lg p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-muted mb-4">{messages[action.type]}</p>
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="text-sm text-muted hover:text-cream px-3 py-1.5 rounded-btn">取消</button>
          <button
            onClick={onConfirm}
            className={`text-sm px-4 py-1.5 rounded-btn ${
              isDanger
                ? 'bg-fav text-cream hover:brightness-110'
